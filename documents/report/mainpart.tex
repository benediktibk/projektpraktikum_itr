...
\begin{equation}
	\sum_{i = 1}^n \vec{^LF_\scr{i}} = m \vec{^W\ddot{p}} + m \vec{^Lg} + \vec{^L\dot{\omega}} \times m \vec{^Lc} + \vec{^L\omega} \times (\vec{^L\omega} \times m \vec{^Lc})
	\label{eq:force_multiple}	
\end{equation}
and
\begin{equation}
	\sum_{i = 1}^n \vec{^LN_\scr{i}} + \sum_{i = 2}^n \vec{^Lr_\scr{i}} \times \vec{^LF_\scr{i}} = \vec{^LI_\scr{p}} \vec{^L\dot{\omega}} + \vec{^L\omega} \times (\vec{^LI_\scr{p}} \vec{^L\omega}) + m \vec{^Lc} \times \vec{^W\ddot{p}} + m \vec{^Lc} \times \vec{^Lg}.
	\label{eq:torque_multiple}	
\end{equation}
The additional summands for the torques are caused by the forces which are not applied in the grasping point one. Implicitly for these formulas we assume that the load coordinate system has its origin in the grasping point of gripper one and that the offsets $\vec{^Lr_\scr{i}}$ from this gripper to the gripper $i$ are known. Additonally, we have again denoted the corresping coordinate systems, but we won't repeat this during the rest of the section for more legible formulas.
...
\begin{equation}
	\vec{h} = 
	\begin{bmatrix}
		\sum_{i = 1}^n \vec{^LF_\scr{i}} \\
		\sum_{i = 1}^n \vec{^LN_\scr{i}} + \sum_{i = 2}^n \vec{^Lr_\scr{i}} \times \vec{^LF_\scr{i}}
	\end{bmatrix}
\end{equation}
