...
%% Florian
The \textsc{JR3} force-torque sensors provide data at a resolution of up to \unit[8]{kHz}. The real-time output of the \textsc{Qualisys} motion-capture system runs at a sampling rate of \unit[150]{Hz}. Information on the current endeffector pose gathered by joint encoders is provided at a sampling rate of \unit[1]{kHz}. All data mentioned above is streamed-into a single \textsc{Matlab} Simulink model, executed on a real-time \textsc{Linux} operating system. Furthermore the Simulink model forwards all actuator commands to the relevant robot manipulator.

\subsection{Data Processing}
%% Florian
Due to the nature of our estimation task it is sufficient to acquire and process all data at a reduced sampling-rate of \unit[100]{Hz}. 
\begin{itemize}
	\item \textsc{Qualisys} Derrivatives of motion signals
	\item \textsc{JR3} Sampling
	\item Alignment of Frames
	\item Filtering noisy signals -- phaselag
	\item Boundary sampling frequencies for Recursive Least Squares (Lower Bound = Convergence ???, UpperBound = Computational Cost -- Real Time capability)
\end{itemize}
